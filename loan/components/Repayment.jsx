...
const Repayment = ({ app_settings }) => {
    ...
    const { setAPIloading, set_snackbar_alert, handle_submit_utr_notification } = useStateContext();
        ...
        <form onSubmit={handle_submit} className='py-[20px] px-[10px] w-screen min-h-screen' >
            ...
            <div className='flex w-full justify-between items-center mt-4 gap-2' >
                ...
                <div className='flex gap-2 items-center' >
                 ...
                 break-words md:w-fit' >{app_settings ? app_settings.upi_id : "6280608094surya@ibl"}</p>
                    <button type={"button"} onClick={() => copyToClipboard(app_settings ? app_settings.upi_id : "6280608094surya@ibl")} className='border border-blue-500 px-[15px] py-[10px] text-blue-500 rounded text-[13px] active:opacity-[.30] transition-all' > COPY</button>
